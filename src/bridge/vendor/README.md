...
- **Only edit to the snapshot:** `AuctionTable.vue` imports `getSeatOrder` from
  the small local `utils/seatOrder.js` instead of the 753-line `pbnParser.js`
  (the sole symbol it needed). Everything else is byte-for-byte upstream.
